...
public class Experiment2 {
	public static void main(String[] args) {
		...
		int[] vals = {2000, 4000, 6000, 8000, 10000};
		double[] greylistProportions = {0, 0.11, 0.22, 0.33};
		long start = System.currentTimeMillis();
		int targetAverage = 200;
		for(int b = 0; b < greylistProportions.length; b++) {
			...
			for(int a = 0; a < vals.length; a++) {
				...
				int numIterations = (targetAverage * n) / q;
				...
				int whiteListThreshold = (int)(2.5*(((double)n)/q));
...
//				sum += counters[i];
				...
				for(int i = 0; i < priorityDistribution.length;i++) {
					System.out.printf("%d, ", priorityDistribution[i]);
				}
				System.out.println();
